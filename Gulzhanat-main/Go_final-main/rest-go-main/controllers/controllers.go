...
func HandleSignup(c *gin.Context) {
	...
	_, err = db.Exec("INSERT INTO users (username, password, role, bank) values ($1, $2, $3, $4)", user.Username, user.Password, "user", 1000)
	if err != nil {
...
func PlaceOrder(c *gin.Context) {
	...
	var price int
	db.QueryRow("select price from items where id=$1", itemId).Scan(&price)
	...
	_, err = db.Exec("UPDATE stats SET items_sold = items_sold + $1, revenue = revenue + $2 where id=1", 1, price)
	if err != nil {
...
func GetRevenue(c *gin.Context) {
	...
	var items_sold int
	var revenue int
	row := db.QueryRow("SELECT items_sold, revenue from stats where id=1")
